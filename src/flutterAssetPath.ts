...
export function findFirstPubspecRoot(startDir: string): string | null {
...
export function readPubspecPackageName(pubspecPath: string): string | null {
  let text: string;
  try {
...
export type FlutterAssetPathContext =
  | { mode: "workspace" }
  | { mode: "package"; packageRoot: string; packageName: string };

/**
 * 若图片目录位于「工作区根 pubspec 以外的子包」内（如 packages/foo/...），
 * 则使用 Flutter 资源路径：packages/<包名>/<相对包根路径>。
 */
export function resolveFlutterAssetPathContext(
  workspaceRoot: string,
  imageRootAbs: string
): FlutterAssetPathContext {
